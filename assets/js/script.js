...
/* Define the slides array/oblect */
const _slides = [
    /// .
    {
...
/* let path_slides = _slides.forEach(getAllImage);

function getAllImage(item) {
    console.log(item.Image);
    return item.Image
}
console.log(path_slides); */
...
for (let i = 0; i < _slides.length; i++) {
    const slidePath = _slides[i].Image;
...
for (let i = 0; i < _slides.length; i++) {
    const thumbPath = _slides[i].Image;
...
setInterval(nextSlide, speedSlide);
